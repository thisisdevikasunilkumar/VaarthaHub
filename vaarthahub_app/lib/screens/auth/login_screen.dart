    ...
    try {
      final response = await http
          .post(
            ...
            body: jsonEncode({
              'emailOrPhone':
                  identifier, // API expects 'emailOrPhone' as the key
              'password': password,
            }),
          )
      ...
      if (response.statusCode == 200) {
        ...
        // --- ID & LOGIN STATUS SAVE LOGIC START ---
        final prefs = await SharedPreferences.getInstance();

        // IMPORTANT: App open aakumbo check cheyyan vendi 'isLoggedIn' true aakkunnu
        ...
        if (role == "Reader" && data['code'] != null) {
          ...
          await prefs.setString('adminCode', (data['code'] ?? 'Admin').toString());
        }
        // --- ID & LOGIN STATUS SAVE LOGIC END ---
      ...
      if (!mounted) return;
      _showFeedback(
        "Connection Error: Please check your server.",
        Colors.redAccent,
      );
    } finally {
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            ...
            right: 0,
            child: Image.asset(
              'assets/ui_elements/element2.png',
              fit: BoxFit.fill,
            ),
          ),
                  ...
                  GestureDetector(
                    onTap: () {
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) =>
                              PasswordRecoveryScreen(emailOrPhone: identifier),
                        ),
                      );
                    },
                    child: const Text(
