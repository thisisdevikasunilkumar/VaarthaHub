...
class RemembranceAdScreen extends StatefulWidget {
  const RemembranceAdScreen({super.key});

  @override
  State<RemembranceAdScreen> createState() => _RemembranceAdScreenState();
}

class _RemembranceAdScreenState extends State<RemembranceAdScreen> {
  ...
  int _selectedDesignIndex = 0; // 0 for Simple, 1 for Medium, 2 for Premium
  ...
  Future<void> fetchCardTypes() async {
    try {
      const String category = 'Remembrance';
      final url = Uri.parse(
  ...
  Widget _buildAdCardPreview({
    ...
    return Stack(
      ...
      children: [
        // 1. The frame itself placed FIRST
        IgnorePointer(
          child:
        ...
        // 2. The uploaded image placed SECOND
  ...
  Widget _buildPhotoUploadField() {
    return Column(
      ...
      children: [
        ...
        const Text(
          '* Passport size or portrait image recommended',
          style: TextStyle(
            color: Colors.grey,
            fontSize: 10,
            fontStyle: FontStyle.italic,
    ...
    return Column(
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            hintText:
                '"e.g. In loving memory of [Name]. Your life was a blessing, your memory a treasure. You are loved beyond words and missed beyond measure. Deeply missed by family and friends.\n\n(നിങ്ങൾക്ക് സന്ദേശം മലയാളത്തിലും ടൈപ്പ് ചെയ്യാവുന്നതാണ്)"',
            ...
            InkWell(
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    const Icon(Icons.add, size: 16, color: Color(0xFF4A65FF)),
                    const SizedBox(width: 5),
                    const Text(
                  ...
                  onPressed: () async {
                    try {
                      ...
                      final File file = File(
                        '${directory.path}/Remembrance_Ad_$timestamp.doc',
                      );
                      ...
                      <html>
                        ...
                        <body>
                          <h2>Remembrance Ad Details</h2>
                          <p><b>Name:</b> ${_nameController.text}</p>
                          <p><b>Date:</b> ${_dateController.text}</p>
                          <p><b>Style:</b> ${_selectedStyle ?? "Not selected"}</p>
                          <p><b>Newspaper:</b> ${_selectedNewspaper ?? "Not selected"}</p>
                          <br/>
                          <h3>Message:</h3>
                          <p>${_messageController.text.replaceAll('\n', '<br/>')}</p>
                          <br/>
                          <p><i>Image Path: ${_selectedImage?.path ?? "No image attached"}</i></p>
                        </body>
                      </html>
                      ''';
